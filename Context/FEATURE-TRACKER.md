...
| Real push notifications (FCM + SW + Cloud Functions) + 3-day reminder + 9am nudge | 🔨 | BRIEF-004 — **PULLED TO PRE-LAUNCH, FULL scope** (Sean, 2026-09-11); target **Wed 16 Sep 09:00 spin-unlock**. Sequence: **merge languages first**, then branch off `main`. No Identity Platform; Manna's approach folded in (no-build). Sean: generate VAPID key now + deploy Functions / publish token rule at hand-off. Includes 3-day reminder, 9am spin nudge, instant shared-photo +5 Function  **BUILT on `feat/push-notifications` at v=49, harness 215/215 (2026-09-12) — NOT merged, nothing deployed.** Full §Scope 1-5. Client FCM behind `ZB_LIVE` + `isSupported()`; hand-written root `firebase-messaging-sw.js` (data-only payloads, SW is the sole renderer, `notificationclick` deep-links via the existing `target`); multi-device tokens at `users/{uid}/fcmTokens/{token}` with send-time cleanup; enabled tracked by TOKEN PRESENCE, not `Notification.permission`; iOS install-first via the A2HS hint. Four Functions (`Context/FUNCTIONS.md`): notification->push (one path for request/accept/message, **localised to the recipient's language**), 3-day reminder, 09:00 Amsterdam nudge (silent before spin-unlock), idempotent instant photo +5 (client self-claim kept). Consent tick default-ON + untickable; persistent two-way **You**-screen toggle (Off deletes the token). **Blocked on Sean:** paste `VAPID_PUBLIC_KEY` (push hides itself while empty), publish the `fcmTokens` rule, `firebase deploy --only functions`. Brief's "SW scope = /" was wrong — this is a project page at `/zb-meetup/`; registration is relative and `getToken` takes the registration. |
