...
| **Icebreakers + tier split by purpose** (68-question bank) | 🔨 | BRIEF-020 built (v=29) on `feat/icebreakers`, tested green — privacy boundary verified (export idea-only, no T2 leak); +10 lands once. **5 polish fixes pending before merge:** explain +10 on the intro; +10 chip on award; You-card spacing; meetup order (photo below Talking points) + make Talking points pop; recap shows icebreakers |
| Primary-button gradient consistency (app-wide) | 📋 | BRIEF-021 — after BRIEF-020; unify CTAs to the Spin gradient, leave secondary buttons as-is |
