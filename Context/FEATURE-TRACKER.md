...
| Real push notifications (FCM + SW + Cloud Functions) + 3-day reminder + 9am nudge | 📋 | BRIEF-004 — **now post-launch fast-follow** (languages take the pre-Monday slot). No Identity Platform needed; Manna's proven approach folded in (no-build adaptation). Sean generates the Web Push VAPID key when we start |
