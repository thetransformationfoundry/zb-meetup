...
// The match stores the English type (it is data, and the export/admin rely on it). This maps it
// to a key for DISPLAY only, falling back to the stored string if it is ever something new.
const TYPE_KEY={"a coffee":"type_coffee","a walk at lunch":"type_walk","a shared break":"type_break",
  "a litter-pick challenge":"type_litter","a Teams coffee call":"type_teams","a virtual catch-up":"type_virtual"};
const typeLabel=ty=>TYPE_KEY[ty]?t(TYPE_KEY[ty]):(ty||'');
...
function howItWorksHTML(inApp){
  ...
  return `<div style="min-height:100vh;background:var(--bg);display:flex;flex-direction:column;">
    <div style="position:sticky;top:0;z-index:20;display:flex;align-items:center;padding:16px 14px;background:rgba(245,247,250,.9);backdrop-filter:blur(12px);"><button type="button" onclick="${inApp?`go('${hiwFrom}')`:`obBackWelcome()`}" style="display:flex;align-items:center;gap:6px;padding:8px 12px 8px 8px;border:0;border-radius:999px;background:transparent;cursor:pointer;color:var(--ink);font-family:inherit;font-size:15px;font-weight:600;">${icon('back',18)}<span>${t('back')}</span></button></div>
    <div style="padding:2px 16px 0;animation:riseIn .5s ease-out both;"><div style="position:relative;overflow:hidden;border-radius:20px;padding:30px 26px 28px;background:linear-gradient(170deg,#3E6EA8 0%,#2F5F9E 42%,#20416F 100%);box-shadow:var(--shadow-lg);color:#fff;"><div style="position:absolute;right:-52px;top:-52px;width:172px;height:172px;border-radius:999px;border:2px dashed rgba(255,255,255,.22);"></div><div style="position:relative;font-size:11.5px;font-weight:700;letter-spacing:1.6px;color:rgba(255,255,255,.72);">${t('hiw_hero_eyebrow')}</div><div style="position:relative;margin-top:10px;font-size:27px;line-height:1.18;font-weight:700;letter-spacing:-.5px;">${t('hiw_hero_h')}</div><div style="position:relative;margin-top:10px;font-size:14.5px;line-height:1.5;color:rgba(255,255,255,.82);max-width:300px;">${t('hiw_hero_sub')}</div></div></div>
    ...
    <div style="position:sticky;bottom:0;padding:14px 16px 22px;background:linear-gradient(to top,#F5F7FA 55%,rgba(245,247,250,0));">
      ...
      <div style="text-align:center;font-size:11.5px;color:var(--muted);margin-top:12px;">${t('zb_only')}</div>
    </div>
  </div>`;
}
...
function viewMeetups(){
  ...
  if(!act.length&&!hist.length)return `<h2>${t('meetups_h')}</h2><p class="sub">${t('meetups_sub')}</p><div class="card center muted">${t('m_none')}</div>`;
  const totalUnread=act.reduce((s,m)=>s+(m.unread||0),0);
  let h=`<h2>${t('meetups_h')}</h2><p class="sub">${t('meetups_sub')}</p>`;
  ...
  act.forEach(m=>{
    if(m.status==='requested'&&m.incoming)h+=`<div class="card"><div class="row"><div class="nicon" style="background:var(--zb-blue-soft)">${icon('users',20)}</div><div style="flex:1"><div style="font-weight:700">${t('m_wants',{name:m.person.name})}</div><div class="muted small">${m.person.role} · ${t('spin_suggested')} ${typeLabel(m.type)}</div></div></div><div class="row" style="gap:10px;margin-top:12px"><button class="btn sm" style="flex:1;justify-content:center" onclick="acceptReq('${m.id}')">${icon('check',16)} ${t('m_accept')}</button><button class="btn ghost sm" style="flex:1;justify-content:center" onclick="declineReq('${m.id}')">${icon('x',16)} ${t('m_decline')}</button></div></div>`;
    else if(m.status==='requested')h+=`<div class="card"><div class="row between"><div class="row">${av(m.person)}<div><div style="font-weight:700">${m.person.name}</div><div class="muted small">${m.person.role}</div></div></div><span class="chip grey">${t('m_waiting')}</span></div></div>`;
    else{const done=canComplete(m);h+=`<div class="card"><div class="row between"><div class="row">${av(m.person)}<div><div style="font-weight:700">${m.person.name}</div><div class="muted small">${m.person.dept}</div></div></div><span class="chip ${done?'good':''}">${done?t('m_ready'):t('m_active')}</span></div><div class="muted small" style="margin:10px 0 4px">${t('m_meetup_is')} <b>${typeLabel(m.type)}</b></div><button class="btn ${done?'secondary':''} sm" style="width:100%;margin-top:8px;justify-content:center" onclick="go('meet:${m.id}')">${done?t('m_review'):t('m_open')}</button></div>`;}
  });
  if(hist.length){h+=`<div class="hr"></div><p class="sub" style="font-weight:700;color:var(--ink)">${t('m_completed')}</p>`;hist.forEach(m=>{h+=`<div class="card" style="cursor:pointer" onclick="go('recap:${m.id}')"><div class="row between"><div class="row">${av(m.person,'sm')}<div><div style="font-weight:600">${m.person.name}</div><div class="muted small">${typeLabel(m.type)}${m.otherCompleted?'':` · ${t('m_waiting_on',{name:m.person.first})}`}</div></div></div><div class="row" style="gap:8px"><span class="chip good">+${myPoints(m)} pts</span>${icon('back',16)}</div></div></div>`;});}
  return h;
}
...
function talkingPointsHTML(m){
  ...
  return `<div class="card talk"><div class="row between"><div class="lead"><span class="nicon">${icon('chat',18)}</span><b>${t('meet_talking')}</b></div><span class="chip">${m.person.first} ${langBadge((fresh&&fresh.lang)||m.person.lang)}</span></div>
    <p class="muted small" style="margin:8px 0 2px">${t('talk_sub',{name:m.person.first})}</p>
...
function viewRecap(id){
  ...
  return `<button class="btn ghost sm" onclick="go('meetups')">${icon('back',16)} ${t('back')}</button>
   ...
   <div class="card"><div class="row between"><div class="row">${av(m.person)}<div><div style="font-weight:700">${m.person.name}</div><div class="muted small">${m.person.role} · ${m.person.dept}</div></div></div><span class="chip good">+${myPoints(m)} pts</span></div>
     <div class="muted small" style="margin-top:10px">${m.photoAwarded?t('recap_photo_pts'):t('recap_no_photo_pts')} · ${m.completed?t('recap_q_pts'):t('recap_q_none')}</div>
     <div class="muted small" style="margin-top:4px">${m.otherCompleted?t('recap_partner_done',{name:m.person.first}):t('recap_partner_not',{name:m.person.first})}</div></div>
   <div class="card"><b>${t('recap_photo')}</b><div style="margin-top:10px">${sceneSquare(sc,'',mp)}</div>${mp?'':`<p class="muted small" style="margin-top:8px">${t('recap_no_photo')}</p>`}
     ...
     <button class="btn secondary sm" style="width:100%;justify-content:center" onclick="addPhoto('${m.id}')">${icon('camera',16)} ${mp?t('ob_photo_change'):t('ob_photo_add')}</button></div>
   ${talkingPointsHTML(m)}
...
function viewMessages(){
  ...
  if(!chats.length)return `<button class="btn ghost sm" onclick="go('meetups')">${icon('back',16)} ${t('back')}</button><h2 style="margin-top:6px">${t('msgs_h')}</h2><p class="sub">${t('msgs_sub')}</p><div class="card center muted">${t('msgs_none')}</div>`;
  let h=`<button class="btn ghost sm" onclick="go('meetups')">${icon('back',16)} ${t('back')}</button><h2 style="margin-top:6px">${t('msgs_h')}</h2><p class="sub">${t('msgs_convos')}</p><div class="card" style="padding:4px 14px">`;
...
function viewAdmin(){
  ...
  return `<button class="btn ghost sm" onclick="go('profile')">${icon('back',16)} ${t('back')}</button><h2 style="margin-top:6px">Admin dashboard</h2><p class="sub">Super-admin view (Donnae + Sean).</p>
